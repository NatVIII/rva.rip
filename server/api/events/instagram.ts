...
async function doOCR(urls: string[]) {
  ...
  const annotationsAll = await Promise.all(
    urls.map(async (url) => {
      ...
      const annotations =
        Object.hasOwn(result, "textAnnotations") &&
...
async function fetchInstagramEvents() {
  ...
  try {
    ...
    instagramOrganizersDb = await Promise.all(
      instagramJson.map(async (instagramOrganizer) => {
        let dbEntry = await prisma.instagramEventOrganizer.findUnique({
          where: { username: instagramOrganizer.username },
        });
        if (!dbEntry) {
    ...
    instagramOrganizersDb = instagramOrganizersDb.sort(
  ...
  try {
    ...
    for (const instagramOrganizerDb of instagramOrganizersDb) {
      let req = await fetch(getInstagramQuery(instagramOrganizerDb.username), {
      ...
      console.log(
        `[IG] Current rate limit: call_count:${callCount} total_cputime:${totalCPUTime} total_time:${totalTime}`
      );
    ...
    eventsZippedAllSources = await Promise.all(
      instagramOrganizersIG.map(async (instagramOrganizer) => {
        return await Promise.all(
          instagramOrganizer.business_discovery.media.data.map(
            async (instagramEvent) => {
              // First check InstagramEvent table.
              const dbEntryEvent = await prisma.instagramEvent.findUnique({
                where: { igId: instagramEvent.id },
              });
              if (dbEntryEvent) {
                return [
                  instagramEvent.id,
              ...
              const dbEntryNonEvent = await prisma.instagramNonEvent.findUnique(
                { where: { igId: instagramEvent.id } }
              );
              return [
                instagramEvent.id,
  ...
  const eventsZippedAllSourcesMap = eventsZippedAllSources.map(
    (eventsZipped) => {
  ...
  try {
    unregisteredInstagramEventsAllSources = eventsZippedAllSources.map(
      (eventsZipped) => {
        // Promise always returns false, so we filter here.
        return eventsZipped
          .filter(([id, eventZip]) => eventZip.dbEntry === null)
          .map(([id, eventZip]) => {
  ...
  let unregisteredInstagramEventsWithOcrAllSources =
    unregisteredInstagramEventsAllSources;
  try {
    unregisteredInstagramEventsWithOcrAllSources = await Promise.all(
      unregisteredInstagramEventsAllSources.map(
        async (unregisteredInstagramEvents) => {
          return await Promise.all(
            unregisteredInstagramEvents.map(async (event) => {
              ...
              switch (event.media_type) {
                ...
                default:
                  console.error(
                    `Unknown media type: ${event.media_type} for event: ${event}`
                  );
                  break;
              }
  ...
  try {
    openAIResponsesAllSources = await Promise.all(
      unregisteredInstagramEventsWithOcrAllSources.map(
        async (unregisteredInstagramEvents, sourceNum) => {
          ...
          return await Promise.all(
            unregisteredInstagramEvents.map(async (event) => {
              ...
              const startPrompt =
                `You're given a post from an Instagram account${
                  tags_string.length > 0 ? " related to " + tags_string : ""
                }. Your task is to parse event information and output it into JSON. (Note: it's possible that the post isn't event-related).\n` +
                ...
                `- The post was posted on ${new Date(
                  event.timestamp
                ).toDateString()}. The time it was posted itself is not an event start time. However, if the post is about an event, the post time can be used to extrapolate event times relative to today, time-relative wording is used; for example, if the event starts 'tomorrow', you can determine that the event begins 1 day after today's date.\n` +
                ...
                `${
                  tags_string.toLowerCase().includes("music")
                    ? "- Add `&` in between multiple music artist names, if any exist.\n"
                    : ""
                }` +
                `${
                  tags_string.toLowerCase().includes("music")
                    ? "- Include featured music artists in the title as well.\n"
                    : ""
                }` +
                `${
                  caption.includes("🎱") || ocrResult.includes("🎱")
                    ? "- Consider 🎱 to be read as `8` if given as a time. \n"
                    : ""
                }` +
              ...
              while (initialGenerationAttempts < maxInitialGenerationAttempts) {
                try {
                  unvalidatedCompletion = {
                    event,
                    data: (await runResponse(startPrompt)).data,
                  };
                  console.log(
                    "output json 1",
                    fixGeneratedJson(
                  ...
                  if (
                    initialGenerationAttempts === maxInitialGenerationAttempts
                  ) {
              ...
              const validationPrompt =
                `You were given a post from an Instagram account and used it to generate the following JSON:\n` +
                "```\n" +
                `${fixGeneratedJson(
                  unvalidatedCompletion.data.choices[0].message.content
                )}` +
                ...
                "```\n" +
                `${
                  event.ocrResult !== undefined
                    ? "OCR Result: " + event.ocrResult
                    : ""
                }` +
                "\n" +
                "```\n" +
              ...
              while (
                validationGenerationAttempts < maxValidationGenerationAttempts
              ) {
                try {
                  ...
                  if (
                    validationGenerationAttempts ===
                    maxValidationGenerationAttempts
                  ) {
                    throw new Error("Could not fetch OpenAI response");
                  }
                }
  ...
  try {
    parsedOpenAIResponsesAllSources = await Promise.all(
      openAIResponsesAllSources.map(async (openAIResponses, sourceNum) => {
        const source = instagramOrganizersDb[sourceNum];
        return Promise.all(
          openAIResponses.map(async ({ event, data }) => {
            ...
            try {
              ...
              if (
                !(
                  Object.hasOwn(potentialResult, "isEvent") &&
              ...
              throw new Error(
                `Caught error: ${e}; could not parse into JSON: ${responseText}`
              );
            }
            // Post-processing.
            ...
            if (
              jsonFromResponse.startMonth === 12 &&
              jsonFromResponse.endMonth === 1
            ) {
            ...
            if (jsonFromResponse.endHourMilitaryTime === null) {
              ...
              jsonFromResponse.endHourMilitaryTime =
                jsonFromResponse.startHourMilitaryTime + 2;
  ...
  try {
    updatedInstagramSources = await Promise.all(
      parsedOpenAIResponsesAllSources.map(
        async (organizerEventsAndNonEventsToAdd, sourceNum) => {
          const source = instagramOrganizersDb[sourceNum];
          const organizerId = source.id;

          // Add each of the new events and non-events to the database.
          let events = await Promise.all(
            organizerEventsAndNonEventsToAdd.map(async (post) => {
                  ...
                  let end = DateTime.fromObject(
                    {
                      ...
                      month: post.endMonth,
                      day: post.startDay,
                      hour: post.endHourMilitaryTime,
                      minute: post.endMinute,
                    },
                      ...
                      start: DateTime.fromObject(
                        {
                          ...
                          day: post.startDay,
                          hour: post.startHourMilitaryTime,
                          minute: post.startMinute,
                        },
                        { zone: "America/Los_Angeles" }
                      )
                        .toUTC()
          ...
          const validEventIds = instagramOrganizersIG[
            sourceNum
          ].business_discovery.media.data.map((post) => post.id);
          // const validEventIds = [];
          const organizerFromDB =
            await prisma.instagramEventOrganizer.findUnique({
              where: {
                id: organizerId,
              },
            });
          ...
          const nonEventsFromOrganizer =
            await prisma.instagramNonEvent.findMany({
              where: {
                organizerId: organizerId,
              },
            });

          const allIdsFromOrganizer = eventsFromOrganizer.concat(
            nonEventsFromOrganizer
          );

          // Delete all events from organizer that are not in eventsToKeepIds.
          await Promise.all(
            allIdsFromOrganizer.map(async (event) => {
              if (!currentEventsIds.has(event.igId)) {
                const isEvent = await prisma.instagramEvent.findFirst({
                  where: { igId: event.igId },
                });
                // Current date in UTC.
                if (isEvent) {
                  // TODO: check that this actually works.
                  // Check if event has actually passed, with a margin of 30 days needing to pass. It's given as UTC.
                  const deleteAfterDate = DateTime.now()
                    .minus({ days: 30 })
                    .toUTC()
                    .toJSDate();
                  const isEventFinished = event.end < deleteAfterDate;
                  console.log(
                    `isEventFinished: ${isEventFinished}, event: ${event.title}`
                  );
                  // Delete events not within a year of current date, in order to avoid clogging database with events that may never happen.
                  const isEventWithinYear =
                    event.start >
                    DateTime.now().minus({ years: 1 }).toUTC().toJSDate();
                  console.log(
                    `isEventWithinYear: ${isEventWithinYear}, event: ${event.title}`
                  );
  ...
  logTimeElapsedSince(
    startTime,
    "Instagram: getting new event sources & pruning"
  );

  const nonUpdatedInstagramOrganizersDb = instagramOrganizersDb.slice(
    firstIndexOfNonUpdatedOrganizer
  );
  ...
  try {
    ...
    console.error(
      "Could return Instagram Events for non-updated sources: ",
      err
    );
    return await useStorage().getItem("instagramEventSources");
  }
...
async function getAllInstagramOrganizers(json) {
  return await Promise.all(
    json.map(async (source) => {
      ...
      return await fetch(instagramQuery, { headers: serverFetchHeaders }).then(
        (res) => {
          const appUsage = res.headers.get("X-App-Usage");
          if (appUsage) {
            console.table(
              `[rate] Instagram; X-App-Usage after ${source.username} is ${appUsage}. Throttled when any reach 100(%).`
            );
          }
          return res.json();
        }
...
function setIgnoreInstagramEventsInplace(
  eventsFromOrganizer: InstagramEvent[]
) {
  for (let i = 0; i < eventsFromOrganizer.length; i++) {
    for (let j = i + 1; j < eventsFromOrganizer.length; j++) {
      if (
        Object.hasOwn(eventsFromOrganizer[i], "display") &&
        eventsFromOrganizer[i].display === "none"
      ) {
        continue;
      }
      ...
      if (
        Object.hasOwn(eventsFromOrganizer[i], "title") &&
        Object.hasOwn(eventsFromOrganizer[j], "title") &&
        (eventsFromOrganizer[i].title === eventsFromOrganizer[j].title ||
          ...
          (eventsFromOrganizer[i].title.length > nameFront &&
            ...
            eventsFromOrganizer[i].title
              .substring(0, nameFront)
              .toLowerCase() ===
              eventsFromOrganizer[j].title
                .substring(0, nameFront)
                .toLowerCase()))
      ) {
      ...
      // Ignore eventsFromOrganizer with the same start time and end time- as they are likely to be the same event.
        ...
        Object.hasOwn(eventsFromOrganizer[i], "start") &&
        Object.hasOwn(eventsFromOrganizer[j], "start") &&
        Object.hasOwn(eventsFromOrganizer[i], "end") &&
        Object.hasOwn(eventsFromOrganizer[j], "end") &&
        eventsFromOrganizer[i].start.toISOString() ===
          eventsFromOrganizer[j].start.toISOString() &&
        eventsFromOrganizer[i].end.toISOString() ===
          eventsFromOrganizer[j].end.toISOString()
      ) {
        eventsFromOrganizer[j].display = "none";
      }
    }
